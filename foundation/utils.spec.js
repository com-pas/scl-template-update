...
import { removeDOsNotInSelection, computeOrphanedRemoves, getMissingMandatoryFields, getEmptyReferencedElements, } from './utils.js';
...
describe('foundation/utils', () => {
    ...
    describe('getEmptyReferencedElements', () => {
        it('returns empty referenced DAType elements', () => {
            const doc = new DOMParser().parseFromString(`<SCL xmlns="http://www.iec.ch/61850/2003/SCL"><DataTypeTemplates>
        ...
        </DataTypeTemplates></SCL>`, 'application/xml');
        ...
        it('does not return empty warnings for paths already flagged as missing mandatory fields', () => {
          ...
          <DOType id="BlkType" cdc="SPS" />
            ...
            const lNodeType = doc.querySelector('LNodeType');
            const missingMandatoryFields = [{ path: ['Blk', 'stVal'], kind: 'subfield' }];
            expect(getEmptyReferencedElements(lNodeType, missingMandatoryFields)).to.deep.equal([]);
        });
    });
});
