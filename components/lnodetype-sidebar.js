...
export class LNodeTypeSidebar extends ScopedElementsMixin(LitElement) {
    ...
    handleClick(id) {
        this.dispatchEvent(new CustomEvent('lnodetype-select', {
            detail: { id },
            bubbles: true,
            composed: true,
        }));
    ...
    get filteredLNodeTypes() {
        ...
        return this.lNodeTypes.filter(ln => {
            ...
            return groups.some(group => group.every(term => id.includes(term) || desc.includes(term)));
        });
    }
    render() {
    ...
    updated(changedProperties) {
        ...
        if (changedProperties.has('lNodeTypes')) {
            ...
            if (oscdList)
                oscdList.scrollTop = 0;
        }
    }
}
LNodeTypeSidebar.scopedElements = {
    'oscd-filled-button': OscdFilledButton,
    'oscd-outlined-textfield': OscdOutlinedTextField,
    'oscd-list': OscdList,
    'oscd-list-item': OscdListItem,
};
LNodeTypeSidebar.styles = css `
    .sidebar {
      display: flex;
...
__decorate([
    property({ type: Array })
], LNodeTypeSidebar.prototype, "lNodeTypes", void 0);
__decorate([
    property({ type: String })
], LNodeTypeSidebar.prototype, "selectedId", void 0);
__decorate([
    state()
], LNodeTypeSidebar.prototype, "filter", void 0);
//# sourceMappingURL=lnodetype-sidebar.js.map