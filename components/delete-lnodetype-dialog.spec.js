...
describe('DeleteDialog', () => {
    ...
    beforeEach(async () => {
        confirmSpy = spy();
        deleteDialog = await fixture(html `<delete-dialog
    ...
    it('displays the correct LNodeType ID in the message', () => {
        deleteDialog.show();
        expect(deleteDialog.shadowRoot?.textContent).to.include('MMXU$oscd$_c53e78191fabefa3');
        expect(deleteDialog.shadowRoot?.textContent).to.include('Confirm delete');
        expect(deleteDialog.shadowRoot?.textContent).to.include('This action may have severe consequences');
    });
    it('should call onConfirm and close when Delete button is clicked', async () => {
        ...
        const buttons = deleteDialog.shadowRoot?.querySelectorAll('oscd-outlined-button');
    ...
    it('should update displayed ID when lnodeTypeId property changes', async () => {
        ...
        await deleteDialog.updateComplete;
        expect(deleteDialog.shadowRoot?.textContent).to.include('LLN0$oscd$_85c7ffbe25d80e63');
        expect(deleteDialog.shadowRoot?.textContent).to.not.include('MMXU$oscd$_c53e78191fabefa3');
    });
...
//# sourceMappingURL=delete-lnodetype-dialog.spec.js.map